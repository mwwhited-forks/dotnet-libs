...
/// </summary>
public class QdrantDatabaseModelFactory : DatabaseModelFactory
{
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    public QdrantDatabaseModelFactory(
...
""";
    ...
    private static (string? Schema, string Table) Parse(string table)
    {
        ...
        if (!match.Success)
        {
            throw new InvalidOperationException(QdrantStrings.InvalidTableToIncludeInScaffolding(table));
        }
        ...
        using (var reader = command.ExecuteReader())
        {
            while (reader.Read())
            {
                ...
                if (supportsMemoryOptimizedTable)
                {
                    ...
                    {
                        table[QdrantAnnotationNames.MemoryOptimized] = true;
                    }
                }
                ...
                if (supportsTemporalTable)
                {
                    ...
                    {
                        table[QdrantAnnotationNames.IsTemporal] = true;

                        var historyTableName = reader.GetValueOrDefault<string>("history_table_name");
                        table[QdrantAnnotationNames.TemporalHistoryTableName] = historyTableName;

                        var historyTableSchema = reader.GetValueOrDefault<string>("history_table_schema");
                        table[QdrantAnnotationNames.TemporalHistoryTableSchema] = historyTableSchema;

                        var periodStartColumnName = reader.GetValueOrDefault<string>("period_start_column");
                        table[QdrantAnnotationNames.TemporalPeriodStartPropertyName] = periodStartColumnName;

                        var periodEndColumnName = reader.GetValueOrDefault<string>("period_end_column");
                        table[QdrantAnnotationNames.TemporalPeriodEndPropertyName] = periodEndColumnName;
                    }
                }
...
#pragma warning restore IDE0034 // Simplify 'default' expression
                ...
                if (isSparse)
                {
                    column[QdrantAnnotationNames.Sparse] = true;
                }
...
ORDER BY [table_schema], [table_name], [index_name], [ic].[key_ordinal];";
        ...
        foreach (var tableIndexGroup in tableIndexGroups)
        {
                ...
                [NotNullWhen(true)] out DatabasePrimaryKey? primaryKey)
            {
                ...
                {
                    primaryKey[QdrantAnnotationNames.Clustered] = false;
                }

                if (primaryKeyGroup.Key.FillFactor is > 0 and <= 100)
                {
                    primaryKey[QdrantAnnotationNames.FillFactor] = (int)primaryKeyGroup.Key.FillFactor;
                }
                ...
                [NotNullWhen(true)] out DatabaseUniqueConstraint? uniqueConstraint)
            {
                ...
                {
                    uniqueConstraint[QdrantAnnotationNames.Clustered] = true;
                }

                if (uniqueConstraintGroup.Key.FillFactor is > 0 and <= 100)
                {
                    uniqueConstraint[QdrantAnnotationNames.FillFactor] = (int)uniqueConstraintGroup.Key.FillFactor;
                }
                ...
                [NotNullWhen(true)] out DatabaseIndex? index)
            {
                ...
                {
                    index[QdrantAnnotationNames.Clustered] = true;
                }

                if (indexGroup.Key.FillFactor is > 0 and <= 100)
                {
                    index[QdrantAnnotationNames.FillFactor] = (int)indexGroup.Key.FillFactor;
                }
