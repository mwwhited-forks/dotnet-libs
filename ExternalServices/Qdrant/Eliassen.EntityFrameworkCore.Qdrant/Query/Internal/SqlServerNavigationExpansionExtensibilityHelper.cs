...
/// </summary>
public class QdrantNavigationExpansionExtensibilityHelper : NavigationExpansionExtensibilityHelper
{
    /// <summary>
    ///     This is an internal API that supports the Entity Framework Core infrastructure and not subject to
    ///     the same compatibility standards as public APIs. It may be changed or removed without notice in
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    public QdrantNavigationExpansionExtensibilityHelper(NavigationExpansionExtensibilityHelperDependencies dependencies)
        : base(dependencies)
    {
    }
            ...
            && !OwnedEntityMappedToSameTableAsOwner(entityType))
        {
            if (!entityType.GetRootType().IsTemporal())
            {
                throw new InvalidOperationException(
                    QdrantStrings.TemporalNavigationExpansionBetweenTemporalAndNonTemporal(entityType.DisplayName()));
            }

            if (source is not TemporalAsOfQueryRootExpression)
            {
                throw new InvalidOperationException(
                    QdrantStrings.TemporalNavigationExpansionOnlySupportedForAsOf("AsOf"));
            }
        }
    ...
    public override bool AreQueryRootsCompatible(EntityQueryRootExpression? first, EntityQueryRootExpression? second)
    {
        ...
        if (firstTemporal || secondTemporal)
        {
            ...
            throw new InvalidOperationException(QdrantStrings.TemporalSetOperationOnMismatchedSources(entityType!.DisplayName()));
        }
