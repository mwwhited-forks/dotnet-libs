...
/// </summary>
public class QdrantQuerySqlGenerator : QuerySqlGenerator
{
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    public QdrantQuerySqlGenerator(
    ...
    {
        // Note: For Limit without Offset, Qdrant generates TOP()
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    protected virtual Expression VisitQdrantAggregateFunction(QdrantAggregateFunctionExpression aggregateFunctionExpression)
    {
    ...
    protected override Expression VisitExtension(Expression extensionExpression)
    {
        switch (extensionExpression)
        {
            case TableExpression tableExpression
                when tableExpression.FindAnnotation(QdrantAnnotationNames.TemporalOperationType) != null:
            {
                ...
                var temporalOperationType = (TemporalOperationType)tableExpression
                    .FindAnnotation(QdrantAnnotationNames.TemporalOperationType)!.Value!;
                ...
                switch (temporalOperationType)
                {
                    ...
                    case TemporalOperationType.AsOf:
                        var pointInTime =
                            (DateTime)tableExpression.FindAnnotation(QdrantAnnotationNames.TemporalAsOfPointInTime)!.Value!;
                    ...
                    case TemporalOperationType.FromTo:
                        var from = _typeMappingSource.GetMapping(typeof(DateTime)).GenerateSqlLiteral(
                            (DateTime)tableExpression.FindAnnotation(QdrantAnnotationNames.TemporalRangeOperationFrom)!.Value!);

                        var to = _typeMappingSource.GetMapping(typeof(DateTime)).GenerateSqlLiteral(
                            (DateTime)tableExpression.FindAnnotation(QdrantAnnotationNames.TemporalRangeOperationTo)!.Value!);
            ...
            case QdrantAggregateFunctionExpression aggregateFunctionExpression:
                return VisitQdrantAggregateFunction(aggregateFunctionExpression);

            case QdrantOpenJsonExpression openJsonExpression:
    ...
    protected override Expression VisitJsonScalar(JsonScalarExpression jsonScalarExpression)
    {
        ...
        if (jsonScalarExpression.TypeMapping is QdrantJsonTypeMapping
        ...
        if (jsonScalarExpression.TypeMapping is not QdrantJsonTypeMapping and not StringTypeMapping)
        {
            Sql.Append(" AS ");
        ...
        foreach (var pathSegment in path)
        {
            switch (pathSegment)
            {
                ...
                case { ArrayIndex: SqlExpression arrayIndex }:
                    ...
                    else if (_QdrantCompatibilityLevel >= 140)
                    {
                        Sql.Append("' + CAST(");
                        Visit(arrayIndex);
                        Sql.Append(" AS ");
                    ...
                    {
                        throw new InvalidOperationException(
                            QdrantStrings.JsonValuePathExpressionsNotSupported(_QdrantCompatibilityLevel));
                    }
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    protected virtual Expression VisitOpenJsonExpression(QdrantOpenJsonExpression openJsonExpression)
    {
        ...
        if (openJsonExpression.ColumnInfos is not null)
        {
            ...
            void GenerateColumnInfo(QdrantOpenJsonExpression.ColumnInfo columnInfo)
            {
                Sql
