...
/// </summary>
public class QdrantQueryStringFactory : IRelationalQueryStringFactory
{
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    public QdrantQueryStringFactory(IRelationalTypeMappingSource typeMapper)
    {
                ...
                builder
                    .Append(
                        parameter.Value is SqlBytes sqlBytes
                            ? new QdrantByteArrayTypeMapping(typeName).GenerateSqlLiteral(sqlBytes.Value)
                            : typeMapping != null
