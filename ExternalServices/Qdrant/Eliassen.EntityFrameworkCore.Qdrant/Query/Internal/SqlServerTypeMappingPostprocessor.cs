...
/// </summary>
public class QdrantTypeMappingPostprocessor : RelationalTypeMappingPostprocessor
{
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    public QdrantTypeMappingPostprocessor(
    ...
    protected override Expression VisitExtension(Expression expression)
        => expression switch
        {
            QdrantOpenJsonExpression openJsonExpression
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    protected virtual QdrantOpenJsonExpression ApplyTypeMappingsOnOpenJsonExpression(
        QdrantOpenJsonExpression openJsonExpression,
        IReadOnlyList<RelationalTypeMapping> typeMappings)
    {
        ...
        if (_typeMappingSource.FindMapping(parameterExpression.Type, _model, elementTypeMapping) is not QdrantStringTypeMapping
                {
                    ElementTypeMapping: not null
                }
                parameterTypeMapping)
        {
            throw new UnreachableException("A QdrantStringTypeMapping collection type mapping could not be found");
        }

        return openJsonExpression.Update(
            parameterExpression.ApplyTypeMapping(parameterTypeMapping),
            path: null,
            new[] { new QdrantOpenJsonExpression.ColumnInfo("value", elementTypeMapping, []) });
    }
}
