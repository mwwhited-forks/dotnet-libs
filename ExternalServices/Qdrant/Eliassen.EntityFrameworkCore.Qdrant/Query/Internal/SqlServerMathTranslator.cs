...
/// </summary>
public class QdrantMathTranslator : IMethodCallTranslator
{
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    public QdrantMathTranslator(ISqlExpressionFactory sqlExpressionFactory)
    {
        ...
        IDiagnosticsLogger<DbLoggerCategory.Query> logger)
    {
        ...
        if (method.DeclaringType == typeof(Math))
        {
            if (method.Name == nameof(Math.Min))
            {
                ...
                throw new InvalidOperationException(QdrantStrings.LeastGreatestCompatibilityLevelTooLow);
            }
