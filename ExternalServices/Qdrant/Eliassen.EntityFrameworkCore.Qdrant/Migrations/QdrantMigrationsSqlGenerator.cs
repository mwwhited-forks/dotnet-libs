...
using Eliassen.EntityFrameworkCore.Migrations.Operations;
...
///     </para>
/// </remarks>
public class QdrantMigrationsSqlGenerator : MigrationsSqlGenerator
{
    ...
    /// <summary>
    ///     Creates a new <see cref="QdrantMigrationsSqlGenerator" /> instance.
    /// </summary>
    /// <param name="dependencies">Parameter object containing dependencies for this service.</param>
    /// <param name="commandBatchPreparer">The command batch preparer.</param>
    public QdrantMigrationsSqlGenerator(
    ...
    protected override void Generate(MigrationOperation operation, IModel? model, MigrationCommandListBuilder builder)
    {
        switch (operation)
        {
            case QdrantCreateDatabaseOperation createDatabaseOperation:
                Generate(createDatabaseOperation, model, builder);
                break;
            case QdrantDropDatabaseOperation dropDatabaseOperation:
        ...
        bool terminate)
    {
        ...
        {
            throw new ArgumentException(QdrantStrings.CannotProduceUnterminatedSQLWithComments(nameof(AddColumnOperation)));
        }
        ...
        MigrationCommandListBuilder builder)
    {
        ...
        if (oldColumnSupported)
        {
            ...
            {
                throw new InvalidOperationException(QdrantStrings.AlterIdentityColumn);
            }
        ...
        if (IsIdentity(operation) && oldColumnSupported)
        {
            ...
            if (TryParseIdentitySeedIncrement(operation, out var newSeed, out _)
                && (operation.OldColumn[QdrantAnnotationNames.Identity] is null
        ...
        var newAnnotations = operation.GetAnnotations().Where(a => a.Name != QdrantAnnotationNames.Identity);
        var oldAnnotations = operation.OldColumn.GetAnnotations().Where(a => a.Name != QdrantAnnotationNames.Identity);
        ...
        if (alterStatementNeeded)
        {
            ...
            definitionOperation.AddAnnotations(
                operation.GetAnnotations().Where(
                    a => a.Name != QdrantAnnotationNames.ValueGenerationStrategy
                        && a.Name != QdrantAnnotationNames.Identity));
        ...
        MigrationCommandListBuilder builder)
    {
        ...
        {
            throw new InvalidOperationException(QdrantStrings.IndexTableRequired);
        }
        ...
        bool terminate = true)
    {
        ...
        {
            throw new ArgumentException(QdrantStrings.CannotProduceUnterminatedSQLWithComments(nameof(CreateTableOperation)));
        }
        ...
        if (operation[QdrantAnnotationNames.IsTemporal] as bool? == true)
        {
            var historyTableSchema = operation[QdrantAnnotationNames.TemporalHistoryTableSchema] as string
            ...
            using (subBuilder.Indent())
            {
                ...
                var startColumnName = operation[QdrantAnnotationNames.TemporalPeriodStartColumnName] as string;
                var endColumnName = operation[QdrantAnnotationNames.TemporalPeriodEndColumnName] as string;
            ...
            var historyTableName = operation[QdrantAnnotationNames.TemporalHistoryTableName] as string;
            string historyTable;
            if (needsExec)
            {
    ...
    /// <summary>
    ///     Builds commands for the given <see cref="QdrantCreateDatabaseOperation" />
    ///     by making calls on the given <see cref="MigrationCommandListBuilder" />.
    /// </summary>
    /// <param name="operation">The operation.</param>
    /// <param name="model">The target model which may be <see langword="null" /> if the operations exist without a model.</param>
    ...
    protected virtual void Generate(
        QdrantCreateDatabaseOperation operation,
        IModel? model,
        MigrationCommandListBuilder builder)
    {
        builder
    ...
    /// <summary>
    ///     Builds commands for the given <see cref="QdrantDropDatabaseOperation" />
    ///     by making calls on the given <see cref="MigrationCommandListBuilder" />.
    /// </summary>
    /// <param name="operation">The operation.</param>
    /// <param name="model">The target model which may be <see langword="null" /> if the operations exist without a model.</param>
    /// <param name="builder">The command builder to use to build the commands.</param>
    protected virtual void Generate(
        QdrantDropDatabaseOperation operation,
        IModel? model,
        MigrationCommandListBuilder builder)
    {
        builder
    ...
    protected override void Generate(
        ...
        MigrationCommandListBuilder builder)
    {
        if (operation[QdrantAnnotationNames.EditionOptions] is string editionOptions)
        {
            builder
                .AppendLine("BEGIN")
    ...
    protected override void Generate(AlterTableOperation operation, IModel? model, MigrationCommandListBuilder builder)
    {
        ...
        {
            throw new InvalidOperationException(QdrantStrings.AlterMemoryOptimizedTable);
        }
        ...
        bool terminate = true)
    {
        ...
        var updateSqlGenerator = (IQdrantUpdateSqlGenerator)Dependencies.UpdateSqlGenerator;
        ...
        MigrationCommandListBuilder builder)
    {
        ...
        if (operation[QdrantAnnotationNames.Sparse] is bool isSparse && isSparse)
        {
            builder.Append(" SPARSE");
        }

        var isPeriodStartColumn = operation[QdrantAnnotationNames.TemporalIsPeriodStartColumn] as bool? == true;
        var isPeriodEndColumn = operation[QdrantAnnotationNames.TemporalIsPeriodEndColumn] as bool? == true;
        ...
        {
            if (operation[QdrantAnnotationNames.TemporalPeriodStartColumnName] is string periodStartColumnName
                && operation[QdrantAnnotationNames.TemporalPeriodEndColumnName] is string periodEndColumnName)
            {
        ...
        var identity = operation[QdrantAnnotationNames.Identity] as string;
        if (identity != null
            || operation[QdrantAnnotationNames.ValueGenerationStrategy] as QdrantValueGenerationStrategy?
    ...
    {
        if (operation[QdrantAnnotationNames.Clustered] is bool clustered)
        {
    ...
    {
        if (operation[QdrantAnnotationNames.Include] is IReadOnlyList<string> includeColumns
    ...
    private static void IndexWithOptions(MigrationOperation operation, MigrationCommandListBuilder builder)
    {
        ...
        if (operation[QdrantAnnotationNames.FillFactor] is int fillFactor)
        {
            options.Add("FILLFACTOR = " + fillFactor);
        }

        if (operation[QdrantAnnotationNames.CreatedOnline] is bool isOnline && isOnline)
        {
            options.Add("ONLINE = ON");
        }

        if (operation[QdrantAnnotationNames.SortInTempDb] is bool sortInTempDb && sortInTempDb)
        {
            options.Add("SORT_IN_TEMPDB = ON");
        }

        if (operation[QdrantAnnotationNames.DataCompression] is DataCompressionType dataCompressionType)
        {
        ...
        MigrationOperation currentOperation)
    {
        ...
        foreach (var index in table.Indexes)
        {
            ...
            else if (index[QdrantAnnotationNames.Include] is IReadOnlyList<string> includeColumns
    ...
    protected virtual bool UseLegacyIndexFilters(CreateIndexOperation operation, IModel? model)
        => (!TryGetVersion(model, out var version) || VersionComparer.Compare(version, "2.0.0") < 0)
            ...
            && operation.IsUnique
            && operation[QdrantAnnotationNames.Clustered] is null or false
    ...
    private static bool IsMemoryOptimized(Annotatable annotatable, IModel? model, string? schema, string tableName)
        => annotatable[QdrantAnnotationNames.MemoryOptimized] as bool?
            ?? model?.GetRelationalModel().FindTable(tableName, schema)?[QdrantAnnotationNames.MemoryOptimized] as bool? == true;

    private static bool IsMemoryOptimized(Annotatable annotatable)
        => annotatable[QdrantAnnotationNames.MemoryOptimized] as bool? == true;

    private static bool IsIdentity(ColumnOperation operation)
        => operation[QdrantAnnotationNames.Identity] != null
            || operation[QdrantAnnotationNames.ValueGenerationStrategy] as QdrantValueGenerationStrategy?
            == QdrantValueGenerationStrategy.IdentityColumn;

    private static bool TryParseIdentitySeedIncrement(ColumnOperation operation, out int seed, out int increment)
    {
        if (operation[QdrantAnnotationNames.Identity] is string seedIncrement
        ...
        MigrationsSqlGenerationOptions options)
    {
        ...
        foreach (var operation in migrationOperations)
        {
            ...
            switch (operation)
            {
                ...
                case DropTableOperation dropTableOperation:
                {
                    var isTemporalTable = dropTableOperation[QdrantAnnotationNames.IsTemporal] as bool? == true;
                    if (isTemporalTable)
                    {
                        ...
                        var historyTableName = dropTableOperation[QdrantAnnotationNames.TemporalHistoryTableName] as string;
                        var historyTableSchema = dropTableOperation[QdrantAnnotationNames.TemporalHistoryTableSchema] as string ?? schema;
                ...
                case RenameTableOperation renameTableOperation:
                {
                    ...
                    var isTemporalTable = renameTableOperation[QdrantAnnotationNames.IsTemporal] as bool? == true;
                    if (isTemporalTable)
                    {
                        DisableVersioning(
                            tableName,
                ...
                case AlterTableOperation alterTableOperation:
                {
                    var isTemporalTable = alterTableOperation[QdrantAnnotationNames.IsTemporal] as bool? == true;
                    var historyTableName = alterTableOperation[QdrantAnnotationNames.TemporalHistoryTableName] as string;
                    var historyTableSchema = alterTableOperation[QdrantAnnotationNames.TemporalHistoryTableSchema] as string ?? schema;
                    var periodStartColumnName = alterTableOperation[QdrantAnnotationNames.TemporalPeriodStartColumnName] as string;
                    var periodEndColumnName = alterTableOperation[QdrantAnnotationNames.TemporalPeriodEndColumnName] as string;

                    var oldIsTemporalTable = alterTableOperation.OldTable[QdrantAnnotationNames.IsTemporal] as bool? == true;
                    var oldHistoryTableName =
                        alterTableOperation.OldTable[QdrantAnnotationNames.TemporalHistoryTableName] as string;
                    var oldHistoryTableSchema =
                        alterTableOperation.OldTable[QdrantAnnotationNames.TemporalHistoryTableSchema] as string
                    ...
                    else
                    {
                        if (oldIsTemporalTable)
                        {
                            ...
                            var oldPeriodStartColumnName =
                                alterTableOperation.OldTable[QdrantAnnotationNames.TemporalPeriodStartColumnName] as string;
                            var oldPeriodEndColumnName =
                                alterTableOperation.OldTable[QdrantAnnotationNames.TemporalPeriodEndColumnName] as string;
                ...
                case AddColumnOperation addColumnOperation:
                {
                    ...
                    if (temporalInformation.IsTemporalTable)
                    {
                        addColumnOperation.RemoveAnnotation(QdrantAnnotationNames.TemporalIsPeriodStartColumn);
                        addColumnOperation.RemoveAnnotation(QdrantAnnotationNames.TemporalIsPeriodEndColumn);
                        ...
                        var isSparse = addColumnOperation[QdrantAnnotationNames.Sparse] as bool? == true;
                ...
                case AlterColumnOperation alterColumnOperation:
                {
                    ...
                    alterColumnOperation.RemoveAnnotation(QdrantAnnotationNames.TemporalIsPeriodStartColumn);
                    alterColumnOperation.RemoveAnnotation(QdrantAnnotationNames.TemporalIsPeriodEndColumn);
                    alterColumnOperation.OldColumn.RemoveAnnotation(QdrantAnnotationNames.TemporalIsPeriodStartColumn);
                    alterColumnOperation.OldColumn.RemoveAnnotation(QdrantAnnotationNames.TemporalIsPeriodEndColumn);
                    ...
                    if (temporalInformation.IsTemporalTable)
                    {
                        if (alterColumnOperation.OldColumn.ComputedColumnSql != alterColumnOperation.ComputedColumnSql)
                        {
                            throw new NotSupportedException(
                                QdrantStrings.TemporalMigrationModifyingComputedColumnNotSupported(
                        ...
                        var changeToSparse = alterColumnOperation.OldColumn[QdrantAnnotationNames.Sparse] as bool? != true
                            && alterColumnOperation[QdrantAnnotationNames.Sparse] as bool? == true;
        ...
        {
            var isTemporalTable = operation[QdrantAnnotationNames.IsTemporal] as bool? == true;
            var historyTableName = operation[QdrantAnnotationNames.TemporalHistoryTableName] as string;
            var historyTableSchema = operation[QdrantAnnotationNames.TemporalHistoryTableSchema] as string ?? schema;
            var periodStartColumnName = operation[QdrantAnnotationNames.TemporalPeriodStartColumnName] as string;
            var periodEndColumnName = operation[QdrantAnnotationNames.TemporalPeriodEndColumnName] as string;
