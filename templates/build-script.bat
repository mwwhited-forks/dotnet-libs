...
SET SOURCE_PROJECTS=%SOURCE_PROJECTS% Nucleus.External.Azure.StorageAccount
...
SET SOURCE_FILES=%SOURCE_FILES% .runsettings
...
SET SOURCE_FILES2=%SOURCE_FILES2% global.json
 
PUSHD %TARGET_PATH%
CALL :NEW_SOLUTION %TARGET_PATH%
CALL :COPY_PROJECT_TO %SOURCE_PROJECTS%
CALL :COPY_PROJECT_TO2 %SOURCE_FOLDERS%
...
ROBOCOPY "%SOURCE_PATH%%TEMP_ITEM%" "%TARGET_PATH%..\%TEMP_ITEM%" /E /MIR /XD obj /XD bin /XO
SHIFT
GOTO :COPY_PROJECT_TO2

:COPY_FILE_TO
SET TEMP_ITEM=%1
IF "%TEMP_ITEM%"=="" EXIT /B
ECHO Copy File: %TEMP_ITEM%
ROBOCOPY "%SOURCE_PATH%%TEMP_ITEM%" "%TARGET_PATH%%TEMP_ITEM%" /XO
SHIFT
GOTO :COPY_FILE_TO
